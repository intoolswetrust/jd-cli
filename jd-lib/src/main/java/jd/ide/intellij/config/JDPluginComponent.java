...
/**
 * Class, which provides basic configuration settings to jd native library. An
 * IJ plugin related code.
 */
public class JDPluginComponent {
	...
	/**
	 * Returns true if a class metadata should be appended to the end of the
	 * decompiled class.
	 * 
	 * @return
	 */
	...
	public boolean isEscapeUnicodeCharactersEnabled() {
		return OptionsManager.getOptions().isEscapeUnicodeCharacters();
	}

	public boolean isOmitPrefixThisEnabled() {
		return OptionsManager.getOptions().isOmitPrefixThis();
	}

	public boolean isRealignLineNumbersEnabled() {
		return OptionsManager.getOptions().isRealignLineNumbers();
	}
